...
Dim Unit_cur_meter As New UnitElementCls
Dim Unit_cur_exameter As New UnitElementCls
    
'Default Property Values:
Const m_def_ctl_FormatSci = 0
Const m_def_ctl_sigDigits = 5

'Property Variables:
Dim m_ctl_FormatSci As Boolean
Dim m_ctl_sigDigits As Integer

Private UnitCol As New Collection

Dim picMouseDown As Boolean

Dim ScriptCtl As Control
Dim ScriptControlRegistered As Boolean

'Event Declarations:
Event TboxKeyPress(KeyAscii As Integer) 'MappingInfo=Text1,Text1,-1,KeyPress
...
Public Property Let ctl_sigDigits(ByVal New_ctl_sigDigits As Integer)
    ...
    UpdateUnits "meter", Unit_cur_meter.curVal
...
'=================[ meter ]
Public Property Get cur_meter() As Double
    cur_meter = Unit_cur_meter.curVal
End Property
Public Property Let cur_meter(ByVal New_cur_meter As Double)
    Unit_cur_meter.curVal = New_cur_meter
    UpdateUnits "meter", Unit_cur_meter.curVal
    PropertyChanged "cur_meter"
End Property

'=================[ exameter ]
Public Property Get cur_exameter() As Double
    cur_exameter = Unit_cur_exameter.curVal
End Property
Public Property Let cur_exameter(ByVal New_cur_exameter As Double)
    Unit_cur_exameter.curVal = New_cur_exameter
    UpdateUnits "exameter", Unit_cur_exameter.curVal
    PropertyChanged "cur_exameter"
End Property
...
Private Sub UserControl_Initialize()
    'set default values
    Unit_cur_meter.defVal = 1
    Unit_cur_exameter.defVal = 1E-18
...
Private Sub UserControl_InitProperties()
    ...
    Unit_cur_meter.curVal = Unit_cur_meter.defVal
    Unit_cur_exameter.curVal = Unit_cur_exameter.defVal
    StartUp
End Sub
...
Private Sub UserControl_ReadProperties(PropBag As PropertyBag)
    ...
    Unit_cur_meter.curVal = PropBag.ReadProperty("cur_meter", Unit_cur_meter.defVal)
    Unit_cur_exameter.curVal = PropBag.ReadProperty("cur_exameter", Unit_cur_exameter.defVal)
    StartUp
End Sub
...
Private Sub UserControl_WriteProperties(PropBag As PropertyBag)
    ...
    Call PropBag.WriteProperty("cur_meter", Unit_cur_meter.curVal, Unit_cur_meter.defVal)
    Call PropBag.WriteProperty("cur_exameter", Unit_cur_exameter.curVal, Unit_cur_exameter.defVal)
End Sub
...
Private Sub LoadUnitColAndComboBox()
    ...
    Unit_cur_meter.uName = "meter": UnitCol.Add Unit_cur_meter, "meter"
    Unit_cur_exameter.uName = "exameter": UnitCol.Add Unit_cur_exameter, "exameter"
    'load combobox
    Combo1.Clear
...
Private Sub UpdateUnits(unitName As String, newVal As Double)
    Dim divisor As Double, ndx As Integer
    divisor = UnitCol(unitName).defVal
    For ndx = 1 To UnitCol.Count
        UnitCol(ndx).curVal = UnitCol(ndx).defVal / divisor * newVal
    Next
    Combo1_Click
End Sub
...
Private Sub Text1_KeyPress(KeyAscii As Integer)
    Dim expr As String, result As String
    ...
    If KeyAscii = 13 Then '[Enter] key.
        ...
        UnitCol(Combo1.ListIndex + 1).curVal = Val(result)
        UpdateUnits UnitCol(Combo1.ListIndex + 1).uName, Val(result)
